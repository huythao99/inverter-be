...
export class MqttService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    ...
    // All instances connect so they can publish (e.g. sync commands to ESP32).
    // Only the primary instance subscribes to device topics to prevent duplicate
    // message processing and double totalA/totalA2 accumulation.
    const instanceId = process.env.NODE_APP_INSTANCE;
    const primaryInstanceId = process.env.MQTT_PRIMARY_INSTANCE || '0';
    const isPrimary =
      instanceId === undefined || instanceId === primaryInstanceId;

    setImmediate(() => {
      this.initializeMqttConnection(isPrimary);
    });

    this.isInitialized = true;
  }

  private initializeMqttConnection(isPrimary: boolean = true) {
